...
export default function AddProductModal({ open, onClose, onRefresh, editData }: AddProductModalProps) {
    ...
    useEffect(() => {
        if (editData) {
            ...
            axios
                .get(`https://edelguur-api.onrender.com/api/productimg/${editData.id}`)
                .then((res) => {
    ...
    useEffect(() => {
        if (!open) return;
        Promise.all([
            fetch("https://edelguur-api.onrender.com/api/category").then((r) => r.json()),
            fetch("https://edelguur-api.onrender.com/api/subcategory").then((r) => r.json()),
            fetch("https://edelguur-api.onrender.com/api/brand").then((r) => r.json()),
            fetch("https://edelguur-api.onrender.com/api/unit").then((r) => r.json()),
            fetch("https://edelguur-api.onrender.com/api/status").then((r) => r.json()),
        ]).then(([cat, sub, br, un, st]) => {
            setCategories(cat);
        ...
        try {
            ...
            if (newImages.length > 0) {
                ...
                const res = await axios.post(
                    "https://edelguur-api.onrender.com/api/productimg/upload-multiple",
                    formData,
            ...
            // 🟢 API дуудах
            if (editData) {
                await axios.put(`https://edelguur-api.onrender.com/api/product/${editData.id}`, payload);
            } else {
                await axios.post("https://edelguur-api.onrender.com/api/product", payload);
            }
